...
Ext2FS::Ext2FS(HDD & disk, unsigned char pnumber) : _hdd(disk), _partition_number(pnumber)
{
	...
	// Load Superblock. Su tamaño es 1024.
	...
	//obtenemos el superbloque a partir de la info del HDD.
...
//dado un inodo, devuelve el número de grupo en el que se encuentra.
...
//devuelve el índice de inodo relativo al grupo donde se encuentra.
...
//dado un número de inodo, queremos devolver el puntero al mismo.
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number){
	...
	unsigned int inode_index_rel = blockgroup_inode_index(inode_number);

	unsigned int inodes_per_block = 1024 / sizeof(struct Ext2FSInode);

	struct Ext2FSBlockGroupDescriptor* group_descriptor = block_group(group_number);
	unsigned int inode_table_dir = group_descriptor->inode_table;

	unsigned int block_number = inode_index_rel / inodes_per_block;
	unsigned int inode_index_in_block = inode_index_rel % inodes_per_block;

	unsigned char *bloque = new unsigned char[1024];
	read_block(inode_table_dir + 1024 * block_number, bloque);


	unsigned char *inode = (unsigned char *)malloc(sizeof(struct Ext2FSInode));
	for (int i = 0; i < 256; i++)
		inode[i] = bloque[inode_index_in_block * sizeof(struct Ext2FSInode) + i];

	Ext2FSInode* res = (Ext2FSInode*)inode;

	return res;
}


//dado un inodo y un número de bloque del inodo. Queremos retornar la dirección de dicho bloque.
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number){
	//tamaño de bloque dividido tamaño direcciones.
	int entry_per_table = 1024 / 4;
	
	bool es_directo = block_number < 12;
	bool es_simple = 12 <= block_number && block_number < 12 + entry_per_table;
	bool es_doble = 12 + entry_per_table <= block_number && block_number < 12 + entry_per_table + entry_per_table * entry_per_table; 
	
	if (es_directo) 	//directo.
		return inode->block[block_number];
	
	if (es_simple){		//indirección simple.
		unsigned int dir_tabla =  inode->block[12];  //desreferenciarlo y llegar al índice block_number - 12.
		unsigned char *buffer_tabla = new unsigned char[1024];
		read_block(dir_tabla, buffer_tabla);
		unsigned int res = buffer_tabla[(block_number - 12) * 4];
		delete(buffer_tabla);
		return res;
	}

	if (es_doble){  //indirección doble. --> ir al puntero
		unsigned int dir_tabla_simples = inode->block[13];	
		unsigned int indice_relativo_simples = (block_number - (12 + entry_per_table)) / entry_per_table;
		unsigned int indice_relativo_directos = (block_number - (12 + entry_per_table)) % entry_per_table;

		unsigned char *tabla_simples = new unsigned char[1024];
		read_block(dir_tabla_simples, tabla_simples);

		unsigned char *tabla_directos = new unsigned char[1024];
		read_block(tabla_simples[indice_relativo_simples], tabla_directos);

		unsigned int res = tabla_directos[indice_relativo_directos];

		delete(tabla_simples);
		delete(tabla_directos);
		return res;
	}
}
