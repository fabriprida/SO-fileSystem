...
//struct para el descriptor del grupo.
...
//stryuct para la entrada de un directorio.
...
struct __attribute__((__packed__)) Ext2FSInode {
	...
	unsigned int dtime;
	unsigned short gid;
	unsigned short links_count;  //hard_links_count
	unsigned int blocks;	//cantidad de bloques?
	unsigned int flags;
	unsigned int os_dependant_1;
	unsigned int block[15];	//punteros a bloques --> 12 directos;  1 indirecto simple; 1 indirecto doble; 1 indirecto
